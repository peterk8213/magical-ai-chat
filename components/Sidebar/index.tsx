...
import { Plus, MessageSquare, Settings, LogOut, X, Trash2, ChevronLeft } from "lucide-react"
...
export function Sidebar({ isOpen, setIsOpen, currentChatId, onNewChat, toggleSidebar, credits }: SidebarProps) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (isOpen && !target.closest('[data-sidebar="true"]') && window.innerWidth < 768) {
        setIsOpen(false)
      }
    }
        ...
        <div className="flex flex-col h-full">
          ...
          <Link href="/profile">
            <div className="p-4 border-b border-zinc-800 flex items-center space-x-3 hover:bg-zinc-800 transition-colors rounded-xl mx-2 mt-2">
              <Avatar>
                <AvatarFallback className="bg-red-600">{username.charAt(0).toUpperCase()}</AvatarFallback>
              </Avatar>
              <div className="flex-1">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {credits < 20 && (
                    <Link href="/buy-credits" className="text-xs text-red-500 hover:text-red-400">
                      Low credits!
                    </Link>
                  )}
                </div>
          ...
          <div className="p-4">
            <Button className="w-full bg-red-600 hover:bg-red-700 text-white rounded-xl" onClick={onNewChat}>
            ...
            <div className="space-y-1 py-2">
              {sortedChats.length > 0 ? (
                sortedChats.map((chat, index) => (
                  <Link key={chat.id} href={`/chat/${chat.id}`} className="block" onClick={() => setIsOpen(false)}>
                    <div
                      className={cn(
                    ...
                    >
                      <div className="flex items-start space-x-2 overflow-hidden">
                        ...
                        <div className="overflow-hidden">
                          ...
                          <div className="text-xs text-zinc-400 truncate">{chat.preview}</div>
                        </div>
                      </div>
                      <Button
                        variant="ghost"
                ...
                <div className="text-center py-4 text-zinc-500 text-sm">No chat history yet</div>
              )}
            </div>
          </ScrollArea>
          ...
          <div className="p-4 border-t border-zinc-800 mt-auto">
            <div className="flex flex-col space-y-2">
              <Button variant="ghost" className="justify-start text-zinc-300 hover:bg-zinc-800 hover:text-white">
