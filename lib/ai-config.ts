...
export const getSystemPrompt = (mode: string, userPreferences: any = null) => {
  // Base system prompt
  let systemPrompt =
    "You are a helpful AI assistant. Format your responses clearly with proper markdown.";
  ...
  if (userPreferences) {
    try {
      ...
      switch (responseLength) {
        ...
        default:
          lengthInstructions =
            "Keep your responses concise and to the point. Short to reduce token usage";
      }
  ...
  switch (mode) {
    case "friend":
      systemPrompt +=
        " You are a friendly and casual AI companion. Use conversational language and be supportive.";
      break;
    case "advisor":
      systemPrompt +=
        " You are a professional advisor. Provide thoughtful, well-structured advice with a formal tone.";
      break;
    case "doctor":
      systemPrompt +=
